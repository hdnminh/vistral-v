...
python -m llava.serve.model_worker --host 0.0.0.0 --controller http://localhost:10000 --port 40000 --worker http://localhost:40000 --model-path /home/lhbac/vic/Vistral-V/scripts/vistral_llava/.checkpoints/llava_vistral-ft &
sleep 20  # Allow more time for the first worker to initialize
...
sleep 20  # Wait longer for the web server to start
